...
class Interconnect extends Module {
  ...
  val in_sel_width = UInt(2 * 4 - 1).getWidth /* bits of number mux select number */

  val out_num = 4 /* output wires number */
  val out_sel_width = UInt(le_num - 1).getWidth /* bits of output mux select number */

  val io = IO(new InterconnectIo)

  val out_sel_sreg = Reg(UInt((out_num * out_sel_width).W))
  val in_sel_sreg = Reg(UInt((le_num * lutx * in_sel_width).W))

  /* TODO: carry sel */

  /* output mux */
  val out_mux = Module(new MuxMtoN(le_num, out_num))
  ...
  /* input mux */
  val in_mux = Module(new MuxMtoN(2 * 4, le_num * lutx))
